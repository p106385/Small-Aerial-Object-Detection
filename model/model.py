...
class UNet(nn.Module):
    def __init__(self):
        ...
        # Encoder
        self.enc1 = nn.Conv2d(64, 128, 3, padding=1)
        self.enc2 = nn.Conv2d(128, 256, 3, padding=1)
        self.enc3 = nn.Conv2d(256, 512, 3, padding=1)

        # Pooling
        self.pool = nn.MaxPool2d(2, 2)

        # Decoder
        self.dec3 = nn.ConvTranspose2d(512, 256, 3, padding=1)
        self.dec2 = nn.ConvTranspose2d(256, 128, 3, padding=1)
        self.dec1 = nn.ConvTranspose2d(128, 64, 3, padding=1)

        # Skip connections
        self.upconv3 = nn.Conv2d(256, 256, 3, padding=1)
        self.upconv2 = nn.Conv2d(128, 128, 3, padding=1)
        self.upconv1 = nn.Conv2d(64, 64, 3, padding=1)

        # Final output layer
        self.out_conv = nn.Conv2d(64, 64, 1)
    ...
    def forward(self, x):
        ...
        dec3 = F.interpolate(dec3, scale_factor=2, mode='bilinear', align_corners=False) + self.upconv3(enc2)
        dec2 = F.relu(self.dec2(dec3))
        dec2 = F.interpolate(dec2, scale_factor=2, mode='bilinear', align_corners=False) + self.upconv2(enc1)
        dec1 = F.relu(self.dec1(dec2))
...
class FinalConv(nn.Module):
    ...
    def forward(self, x):
        x = self.lrelu(self.upconv1(F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=False)))
        x = self.lrelu(self.upconv2(F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=False)))
...
class ESN(nn.Module):
    def __init__(self):
        ...
        self.beginEdgeConv = BeginEdgeConv()  # Convolution layers for edge detection
        self.u_net = UNet()                  # U-Net for enhanced edge extraction
        self.maskConv = MaskConv()            # Mask layer for edge sharpening
        self.finalConv = FinalConv()          # Final upsampling and convolution layer

    def forward(self, x):
        x_lap = kornia.filters.sobel(x)       # Sobel filter for edge extraction
        x1 = self.beginEdgeConv(x_lap)
        x2 = self.u_net(x1)                   # U-Net to enhance edge features
        x3 = self.maskConv(x2)
        x4 = x3 * x2 + x2                     # Element-wise multiplication for edge enhancement
...
class FPGAN_ESN(nn.Module):
  def __init__(self, in_nc, out_nc, nf, nb):
    super(FPGAN_ESN, self).__init__()
    self.netRG = RRDBNet(in_nc, out_nc, nf, nb)
    self.netE = ESN()

  def forward(self, x):
    x_base = self.netRG(x)  
    x5, x_lap = self.netE(x_base) # ESN net
